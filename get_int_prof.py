...
def get_ac_int_site_v3(outdir, runname, dep_sample):
    ...
    exchac_int = linave(dep, (1 - poro) * dense * cec * cecfrac, dep_sample) / linave(
        dep, (1 - poro) * dense * cec, dep_sample
    )
...
def get_totsldwt_site(outdir, runname, dep_sample):
    ...
    for sp in sps:
        isp = sp_list.index(sp)
        conc_int = linave(dep, (1 - poro) * dense * data[:, isp], dep_sample) / linave(
            dep, (1 - poro) * dense, dep_sample
        )
...
def get_DIC_save_get_site_OLD(outdir, runname, dep_sample):
    ...
    temp = (
        1.0 / (np.log(k2 / k2_ref) * (-Rg / H_k2) + 1.0 / (temp_ref + tempk_0))
        - tempk_0
    )
    ...
    H_k1 = 11.94453
    k1 = k1_ref * np.exp(
        -H_k1 / Rg * (1.0 / (temp + tempk_0) - 1.0 / (temp_ref + tempk_0))
    )
    ...
    np.savetxt(
        outdir + runname + "/prof/prof_aq-DIC-{:03d}_OLD.txt".format(i),
        np.transpose(np.array([list(deps), list(DIC)])),
    )
...
def get_DIC_save_get_site_NEW(outdir, runname, dep_sample, i_save):
    ...
    np.savetxt(
        outdir + runname + "/prof/prof_aq-DIC-{:03d}_NEW.txt".format(i_save),
        np.transpose(np.array([list(deps), list(DIC)])),
    )
...
def get_ave_DIC_save(outdir, runname, dep_sample):
    ...
    DIC_btm = np.average(
        DIC[: idep + 1] * poro[: idep + 1] * sat[: idep + 1] * 1e3
    ) / np.average(  # mol/ soil m3
        (1 - poro[: idep + 1])
    )  # mol/ solid m3

    np.savetxt(
        outdir + runname + "/prof/prof_aq-DIC-{:03d}.txt".format(i),
        np.transpose(np.array([list(deps), list(DIC)])),
    )
...
def get_ave_DIC_save_v2(outdir, runname, dep_sample):
    ...
    DIC_ave = linave(dep, DIC * poro * sat * 1e3, dep_sample) / linave(  # mol/ soil m3
        dep, (1 - poro), dep_sample
    )  # mol/ solid m3

    np.savetxt(
        outdir + runname + "/prof/prof_aq-DIC-{:03d}.txt".format(i),
        np.transpose(np.array([list(dep), list(DIC)])),
    )
...
def get_ave_IS(outdir, runname, dep_sample):
    ...
    IS_ave = np.average(IS[: idep + 1])  # mol/ L
...
def get_waterave_site(outdir, runname, dep_sample):
    ...
    for sp in sps:
        ...
        btmconcs.append(
            np.average(
                data[: idep + 1, isp] * poro[: idep + 1] * sat[: idep + 1] * 1e3
            )  # mol/ soil m3
